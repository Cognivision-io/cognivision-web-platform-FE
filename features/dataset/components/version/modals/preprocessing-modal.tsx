...
const PREPROCESSING_OPTIONS = [
  { 
    ...
    icon: Zap, 
    upgrade: false,
    description: "Discard EXIF rotation",
    detail: "Applied"
  },
  { 
    name: "Grayscale", 
  ...
  { 
    ...
    detail: "Stretch to 200x200"
  },
];

export const PreprocessingModal = ({ isOpen, onClose, onAdd, existingSteps }: PreprocessingModalProps) => {
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl bg-white p-0 overflow-hidden border-none shadow-2xl">
        <DialogHeader className="p-6 border-b border-slate-100">
           <div className="flex items-center gap-2">
              <Zap className="h-5 w-5 text-[#6841ff]" />
              <DialogTitle className="text-xl font-bold text-slate-800">Preprocessing Options</DialogTitle>
           </div>
