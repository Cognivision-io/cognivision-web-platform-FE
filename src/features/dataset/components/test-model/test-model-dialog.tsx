...
// Roboflow Publishable API Key from environment variable
const ROBOFLOW_PUBLISHABLE_KEY = process.env.NEXT_PUBLIC_ROBOFLOW_API_KEY || "rf_SHcRxszOu2Q1WcoxymMCXBp1CEC3";
...
export const TestModelDialog = ({ version, project, children }: TestModelDialogProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedImage, setSelectedImage] = useState<string | null>(null);
  const [confidenceThreshold, setConfidenceThreshold] = useState(50);
  const [overlapThreshold, setOverlapThreshold] = useState(50);
  const [inferenceResult, setInferenceResult] = useState<RoboflowInferenceResponse | null>(null);
  const apiKey = ROBOFLOW_PUBLISHABLE_KEY;
  ...
  const runInference = async (imageUrl: string) => {
    ...
    try {
      ...
      const result = await runRoboflowInference(
        ...
        confidenceThreshold,
        overlapThreshold,
        version.model.endpoint // Pass the custom endpoint
      );
        ...
        <div className="border-b px-6 py-4 flex items-center justify-between bg-white shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
                ...
                <Select defaultValue={version.id}>
                    ...
                    <SelectContent>
                        <SelectItem value={version.id}>{version.model?.id || version.name}</SelectItem>
                    </SelectContent>
                </Select>
            </div>
          </div>
          <div className="flex items-center gap-6 text-sm">
             <div className="flex flex-col items-end">
                <span className="text-xs text-slate-500 uppercase font-medium">mAP@50</span>
                <span className="font-semibold text-[#6841ff]">{version.model?.map || "0.0"}%</span>
             </div>
             <div className="flex flex-col items-end">
                <span className="text-xs text-slate-500 uppercase font-medium">Precision</span>
                <span className="font-semibold text-slate-700">{version.model?.precision || "0.0"}%</span>
             </div>
             <div className="flex flex-col items-end">
                <span className="text-xs text-slate-500 uppercase font-medium">Recall</span>
                <span className="font-semibold text-slate-700">{version.model?.recall || "0.0"}%</span>
             </div>
          </div>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden bg-[#f8f9fc]">
          ...
          <div className="w-80 border-r bg-white p-4 flex flex-col gap-6 overflow-y-auto shrink-0">
             ...
             <div className="space-y-2">
                <h3 className="font-medium text-sm text-slate-900">Upload Image or Video File</h3>
                <div 
                    {...getRootProps()} 
                    className={cn(
                        ...
                        isEngineReady && !isDragActive && "cursor-pointer border-slate-200 hover:border-[#6841ff]/50",
                ...
                >
                    ...
                    {!isEngineReady ? (
                      <>
                        ...
                        <p className="text-xs text-slate-500 text-center">Loading model...</p>
                      </>
                    ) : (
                      <>
                        <Upload className="h-6 w-6 text-slate-400" />
                        <p className="text-xs text-slate-500 text-center">Drop file here or</p>
                        <Button variant="outline" size="sm" className="h-7 text-xs">Select File</Button>
                      </>
                    )}
                </div>
             </div>
             ...
             {/* Paste URL */}
             <div className="space-y-2">
                <h3 className="font-medium text-sm text-slate-900">Paste Image URL</h3>
                <div className="flex gap-2">
                    <div className="relative flex-1">
                        <LinkIcon className="absolute left-2.5 top-2.5 h-4 w-4 text-slate-400" />
                        <Input 
                          className="pl-9 h-9 text-xs" 
                          placeholder={isEngineReady ? "Paste a link..." : "Loading model..."} 
          ...
          <div className="flex-1 flex items-center justify-center p-6 overflow-hidden relative">
            {selectedImage ? (
                <div className="relative max-w-full max-h-full shadow-lg rounded-lg overflow-hidden border bg-white inline-block">
                   ...
                   {/* Overlay Predictions */}
                   {inferenceResult && inferenceResult.predictions && inferenceResult.predictions.map((pred, i) => {
                      // Filter by threshold logic just for display
                      if(pred.confidence < confidenceThreshold/100) return null;
                   ...
                   {isInferencing && (
                       <div className="absolute inset-0 bg-white/50 flex items-center justify-center z-20">
                           <div className="flex flex-col items-center gap-2">
                               ...
                               <span className="text-xs font-semibold text-[#6841ff]">Running Inference...</span>
                           </div>
                       </div>
                   )}
                </div>
                ...
                <div className="text-center space-y-4">
                    ...
                    <div>
                        <h3 className="text-lg font-medium text-slate-900">No Image Selected</h3>
                        <p className="text-slate-500">Select an image from the sidebar to test the model.</p>
                    </div>
                </div>
            )}
            ...
            {/* Object count badge */}
             {inferenceResult && inferenceResult.predictions && (
                <div className="absolute bottom-6 right-6 bg-slate-900 text-white text-xs px-3 py-1.5 rounded-md shadow-md z-30">
                   {inferenceResult.predictions.filter((p) => p.confidence >= confidenceThreshold/100).length} object{inferenceResult.predictions.filter((p) => p.confidence >= confidenceThreshold/100).length !== 1 ? 's' : ''} detected
                </div>
             )}
          </div>
          ...
          <div className="w-80 border-l bg-white flex flex-col shrink-0">
             ...
             <div className="p-4 space-y-6 border-b">
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                         <label className="text-xs font-medium text-slate-700">Confidence Threshold</label>
                         <span className="text-xs text-slate-500">{confidenceThreshold}%</span>
                    </div>
                    <Slider 
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                         <label className="text-xs font-medium text-slate-700">Overlap Threshold</label>
                         <span className="text-xs text-slate-500">{overlapThreshold}%</span>
                    </div>
                    <Slider 
                ...
                <div className="space-y-2">
                     <label className="text-xs font-medium text-slate-700">Label Display Mode</label>
             ...
             <div className="flex-1 flex flex-col overflow-hidden">
                <div className="p-3 bg-slate-50 border-b flex items-center justify-between">
                     <span className="text-xs font-medium text-slate-600">Inference Response</span>
                ...
                <div className="flex-1 p-4 overflow-auto bg-white">
                    <pre className="text-[10px] font-mono text-slate-600 leading-tight">
                        {inferenceResult ? JSON.stringify(inferenceResult.predictions, null, 2) : "No inference run yet..."}
                    </pre>
                </div>
             </div>
