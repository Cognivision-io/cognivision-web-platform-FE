...
export const CreateVersionScreen = ({
  ...
  const [versionName, setVersionName] = useState(format(new Date(), "yyyy-MM-dd h:mma").toLowerCase());
    ...
    augmentationSteps.forEach((step: any) => {
      ...
      switch (key) {
        ...
        case 'hue':
          ...
          payload.augmentation['hue'] = { degrees: hMatch ? parseInt(hMatch[1]) : 180 };
          break;
        case 'exposure':
          const eMatch = step.detail.match(/Percent: (\d+)%/);
          payload.augmentation['exposure'] = { percent: eMatch ? parseInt(eMatch[1]) : 30 };
          break;
        case 'blur':
          const blMatch = step.detail.match(/Pixels: ([\d.]+)/);
          payload.augmentation['blur'] = { pixels: blMatch ? parseFloat(blMatch[1]) : 1.5 };
          break;
        case 'saturation':
          const sMatch = step.detail.match(/Percent: (\d+)%/);
          payload.augmentation['saturation'] = { percent: sMatch ? parseInt(sMatch[1]) : 50 };
          break;
        case 'noise':
          const nMatch = step.detail.match(/Percent: (\d+)%/);
          payload.augmentation['noise'] = { percent: nMatch ? parseInt(nMatch[1]) : 50 };
          break;
        case 'rotation':
          const rotMatch = step.detail.match(/(-?\d+).*?(-?\d+)/);
          payload.augmentation['rotate'] = { degrees: rotMatch ? parseInt(rotMatch[2]) : 45 };
          break;
        case 'crop':
  ...
  return (
    <div className="flex-1 flex flex-col bg-white overflow-hidden">
      ...
      <div className="px-8 py-4 border-b border-slate-200 flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={onBack} className="text-slate-500 hover:text-slate-700">
        ...
        <div className="max-w-4xl mx-auto space-y-12">
          ...
          <div>
            ...
            <div className="space-y-1.5 max-w-md">
              <label className="text-sm font-semibold text-slate-700">Version Name:</label>
              <Input 
                value={versionName}
          ...
          <div className="space-y-10">
            ...
            <div className="flex gap-6">
              ...
              <div className="pb-4">
                ...
                <div className="grid grid-cols-[120px_1fr] gap-y-2 text-sm">
                  <span className="font-bold text-slate-700">Training Set:</span>
                  <span className="text-slate-600">15 images</span>
                  <span className="font-bold text-slate-700">Validation Set:</span>
            ...
            <div className="flex gap-6">
              <div className="flex flex-col items-center">
                 <div className={cn(
                   ...
                   currentStep > 3 ? "bg-[#6841ff] border-[#6841ff] shadow-lg shadow-[#6841ff]/20" : 
                   currentStep === 3 ? "border-[#6841ff] text-[#6841ff]" : "border-slate-200 text-slate-300"
                 )}>
                   {currentStep > 3 ? <Check className="h-5 w-5 text-white" /> : "3"}
                 </div>
                 <div className="w-0.5 h-full bg-slate-100 mt-2"></div>
              </div>
              <div className="flex-1 pb-4">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-lg font-bold text-slate-900">Preprocessing</h3>
                     ...
                     <div className="border border-slate-200 rounded-lg overflow-hidden divide-y divide-slate-100 bg-white shadow-sm">
                        {preprocessingSteps.map(step => (
                           <div key={step.id} className="p-4 flex items-center justify-between group">
                              <div>
                                <div className="font-bold text-slate-700 text-sm">{step.name}</div>
                                <div className="text-slate-500 text-xs">{step.detail}</div>
                              </div>
                              <div className="flex items-center gap-3">
                                 <button className="text-[#6841ff] text-xs font-semibold hover:underline border-none bg-transparent cursor-pointer">Edit</button>
                                 <button onClick={() => removePreprocessingStep(step.id)} className="text-slate-300 hover:text-slate-500 transition-colors border-none bg-transparent cursor-pointer">
                                    <X className="h-4 w-4" />
                                 </button>
                              </div>
                           </div>
                        ...
                        >
                           ...
                           <span className="text-sm font-semibold">Add Preprocessing Step</span>
                        </button>
                     </div>
                     <Button onClick={handleContinue} className="bg-[#6841ff] hover:bg-[#5936db] text-white px-8 h-10 font-bold rounded-md shadow-md">
                        Continue
                     </Button>
                  </div>
                )}
             ...
             <div className="flex gap-6">
              <div className="flex flex-col items-center">
                 <div className={cn(
                   "h-10 w-10 rounded-full flex items-center justify-center border-2 transition-all",
                   currentStep > 4 ? "bg-[#6841ff] border-[#6841ff] shadow-lg shadow-[#6841ff]/20" : 
                   currentStep === 4 ? "border-[#6841ff] text-[#6841ff]" : "border-slate-200 text-slate-300"
                 )}>
                   {currentStep > 4 ? <Check className="h-5 w-5 text-white" /> : "4"}
                 </div>
                 <div className="w-0.5 h-full bg-slate-100 mt-2"></div>
              </div>
              <div className="flex-1 pb-4">
                 <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-lg font-bold text-slate-900">Augmentation</h3>
                  <button className="text-slate-400 hover:text-slate-600 border-none bg-transparent cursor-pointer">
                    <Info className="h-4 w-4" />
                  </button>
                </div>
                   ...
                   <div className="space-y-4 max-w-md">
                     <div className="space-y-3">
                        {augmentationSteps.length > 0 ? (
                           augmentationSteps.map((step: any) => (
                              <div key={step.id} className="flex items-center gap-3 group">
                                 ...
                                 <button onClick={() => removeAugmentationStep(step.id)} className="text-slate-300 hover:text-slate-500 transition-colors border-none bg-transparent cursor-pointer">
                                    <X className="h-4 w-4" />
                                 </button>
                              </div>
                           ))
                        ...
                        <div className="border border-slate-200 rounded-lg overflow-hidden bg-white shadow-sm mt-4">
                           ...
                           >
                              <Plus className="h-4 w-4 transition-transform group-hover:scale-110" />
                              <span className="text-sm font-semibold">Add Augmentation Step</span>
                           </button>
                           <button className="w-full p-4 flex items-center gap-3 text-slate-600 hover:bg-slate-50 transition-colors group border-none bg-transparent cursor-pointer text-left">
                              ...
                              <div className="flex flex-col">
                                 <span className="text-sm font-semibold text-slate-700">Use Previous Augmentations</span>
                                 <span className="text-[11px] text-slate-500">Use augmentations from a previous version.</span>
                              </div>
                           </button>
                        </div>
                     </div>
                     <div className="flex items-center gap-3 mt-6">
                        ...
                        <Button variant="outline" onClick={() => setAugmentationSteps([])} className="border-slate-200 text-slate-600 h-10 font-bold px-6 bg-white hover:bg-slate-50">
                           Clear All
                        </Button>
                     </div>
                  </div>
                ...
                {currentStep > 4 && (
                   <div className="space-y-1 text-sm bg-slate-50 p-4 rounded-lg border border-slate-100 max-w-md">
                      {augmentationSteps.length > 0 ? (
                        ...
                        <span className="text-sm text-slate-500 italic">None applied</span>
                      )}
                   </div>
                )}
              </div>
            ...
            <div className="flex gap-6">
              <div className="flex flex-col items-center">
                 <div className={cn(
                   "h-10 w-10 rounded-full flex items-center justify-center border-2 transition-all",
                   currentStep === 5 ? "border-[#6841ff] text-[#6841ff]" : "border-slate-200 text-slate-300"
                 )}>
      ...
      <PreprocessingModal 
        ...
        onAdd={(step) => setPreprocessingSteps([...preprocessingSteps, { ...step, id: Math.random().toString() }])}
      ...
      <AugmentationModal 
        ...
        onAdd={(step) => setAugmentationSteps([...augmentationSteps, { ...step, id: Math.random().toString() }] as any)}
